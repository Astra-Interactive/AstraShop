...
object PriceCalculator {
    ...
    private fun transform(item: ShopConfig.ShopItem<*>, price: Double, amount: Int): Double {
    ...
    fun calculateBuyPrice(item: ShopConfig.ShopItem<*>, amount: Int): Double {
        return (amount downTo 1).sumOf { transform(item, item.buyPrice, amount).coerceAtLeast(item.sellPrice) }.round()
    }

    fun calculateSellPrice(item: ShopConfig.ShopItem<*>, amount: Int): Double {
        return ((amount downTo 1).sumOf { transform(item, item.sellPrice, amount).coerceAtMost(item.priceMin) }).round()
    }
}
